...
def pick_widget(col, dtype, unique_vals, val_range, real_labels=None):
    ...
    # Real labels from uploaded CSV  →  labeled dropdown
    if real_labels:
        opts = [{"value": str(k), "label": str(v)}
                for k, v in sorted(real_labels.items(), key=lambda x: x[0])]
        return {"widget": "labeled_dropdown", "options": opts}

    # Boolean toggle (only values 0 and 1)
    if unique_vals and len(unique_vals) <= 2 and \
       set(str(v).lower() for v in unique_vals) <= {'0', '1'}:
        return {"widget": "toggle"}

    # Object / string categorical
    if "object" in dtype or ("category" in dtype and
       any(isinstance(v, str) for v in unique_vals)):
        if n <= 25:
            return {"widget": "dropdown",
                    "options": [str(v) for v in sorted(unique_vals, key=str)]}
        return {"widget": "text"}

    # Numeric with few integer values  →  encoded category dropdown
    if ("int" in dtype or "float" in dtype or "category" in dtype) and 2 < n <= 20:
        try:
            ...
            if all(float(v) == int(float(v)) for v in unique_vals):
                opts = [{"value": str(v), "label": f"Category {v}"} for v in nums]
...
def extract_csv_labels(csv_df: pd.DataFrame, encoded_df: pd.DataFrame,
    ...
    """
    Build {col: {int_code: original_label}} mapping by aligning
    the sanitised CSV with AutoGluon's encoded training DataFrame.
    """
    labels  = {}
    exclude = {target_col} | set(added_features.keys()) | set(added_features.values())
    ...
    for col in encoded_df.columns:
        ...
        enc = encoded_df[col].dropna()
        raw = csv_df[col].dropna()
        enc_dtype = str(encoded_df[col].dtype)
        csv_dtype = str(csv_df[col].dtype)

        # Case 1: encoded is numeric but CSV is string  →  align by position
        if (("int" in enc_dtype or "float" in enc_dtype) and
                ("object" in csv_dtype or "category" in csv_dtype)):
            if len(enc) == len(raw):
                try:
                    mapping = {}
                    for e_val, c_val in zip(enc.values, raw.values):
                        k = int(float(e_val))
                        if k not in mapping:
                            mapping[k] = str(c_val)
                    if mapping:
        ...
        # Case 2: encoded is category dtype with integer categories
        elif "category" in enc_dtype:
            try:
                cats = encoded_df[col].cat.categories.tolist()
                if all(isinstance(c, (int, float, np.integer, np.floating))
                       for c in cats):
                    unique_csv = [v for v in csv_df[col].dropna().unique()
                                  if v == v]
                    if len(cats) == len(unique_csv):
                        labels[col] = {
                            int(c): str(u)
                            for c, u in zip(sorted(cats),
                                            sorted(unique_csv, key=str))
                        }
            except Exception:
                pass
...
async def upload_zip(file: UploadFile = File(...)):
    ...
    try:
        ...
        if not ag_dir:
            raise HTTPException(400, "AutoGluon model not found — no predictor.pkl in ZIP")
        ...
        try:
            ...
            raise HTTPException(500, f"AutoGluon load failed: {e}")
