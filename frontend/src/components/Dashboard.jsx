...
const TABS = [
  ...
  { id: 'profile',     label: 'Data Profile',icon: <IconProfile /> },
...
function IconProfile()     { return <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="3" width="18" height="18" rx="2"/><path d="M3 9h18M9 21V9"/></svg> }
    ...
    <div className="relative z-10 flex h-screen overflow-hidden">
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
          ...
          <div className="flex items-center gap-4">
            <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center gap-2 mt-0.5 flex-wrap">
                ...
                {filesFound.has_profile_report && (
                  <button
                    onClick={() => setActiveTab('profile')}
                    className="text-cyan-700 hover:text-cyan-400 font-mono text-xs flex items-center gap-1 transition-colors"
                    title="View Data Profile Report"
                  >
                    📊 Profile Report
                  </button>
                )}
        ...
        <nav className="flex gap-0.5 px-4 pt-3 pb-0 border-b border-border flex-shrink-0 overflow-x-auto"
          ...
          {TABS.map(tab => {
            const showDot = tab.id === 'profile' && filesFound.has_profile_report
            return (
            <button
              key={tab.id}
              onClick={() => setActiveTab(tab.id)}
              className={`relative flex items-center gap-2 px-4 py-2.5 text-xs font-body font-medium border-b-2 transition-all duration-200 whitespace-nowrap rounded-t-lg
                ${activeTab === tab.id
        ...
        <div className="flex-1 overflow-y-auto">
          <div className="p-6">
            ...
            {activeTab === 'profile'     && <ProfileReport deployment={activeDeployment} />}
